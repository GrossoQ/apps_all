...
async def telnet_to_olt(olt, puerto, posicion): # OLT Inicial
    ...
    try:
        ...
        patron_name_tel = r"name\s+(.+?)\s+TEL\.\s+(.+)"
        ...
        if name_tel:
            ...
            return mostrar_texto("No datos name, tel")
        ...
        if sn:
            ...
            return mostrar_texto("No datos SN")
        
        
        # Ejecuta comando usuario contraseña
        writer.write(f"show onu running config gpon-onu_{puerto}:{posicion}\n")
        await writer.drain()  # Espera a que se envíe el comando
        # Leer hasta que se encuentre el delimitador "#"
        data = await asyncio.wait_for(reader.read(20000), timeout=10)
        # Decodificar los datos leídos como una cadena
        datos_str = await asyncio.wait_for(reader.read(20000), timeout=10)
        ...
        if resultado_usuario_contraseña:
            ...
            return mostrar_texto("No datos user, pass")
